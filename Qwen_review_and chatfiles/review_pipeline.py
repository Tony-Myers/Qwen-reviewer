...
_METHOD_PRIORITY: Dict[MethodClass, int] = {
    ...
    MethodClass.BAYESIAN_MODEL: 4,
    MethodClass.DISTRIBUTIONAL_MODEL: 5,
    MethodClass.FREQUENTIST_MIXED_EFFECTS: 6,
    MethodClass.FREQUENTIST_ROBUST: 7,
    MethodClass.FREQUENTIST_ALLOMETRIC: 8,
    MethodClass.FREQUENTIST_PROFILE_MODEL: 9,
    MethodClass.PREDICTIVE_ML: 10,
    MethodClass.FREQUENTIST_ANOVA: 11,
    MethodClass.FREQUENTIST_REGRESSION: 12,
    MethodClass.FREQUENTIST_CORRELATION: 13,
    MethodClass.FREQUENTIST_SIMPLE_COMPARISON: 14,
    MethodClass.DESCRIPTIVE: 15,
    MethodClass.MIXED_METHODS: 16,
...
def classify_additional_methods(full_text: str, primary: MethodClass) -> List[MethodClass]:
    ...
    if (
        ...
        if primary != MethodClass.BAYESIAN_MIXED_EFFECTS and primary != MethodClass.BAYESIAN_MODEL:
    ...
    # Correlation (only flag as additional if not already primary)
    if re.search(
        r"\bpearson'?s?\b|\bspearman'?s?\b|\bcorrelation\s+coefficien",
        text_for_classification,
    ):
    ...
    if re.search(
        r"linear\s+regress|logistic\s+regress|multiple\s+regress|\bols\b|\bglm\b(?!.*gamlss)",
        text_for_classification,
    ):
...
def detect_inline_tables(text: str) -> List[dict]:
    ...
    while i < len(lines):
        ...
        if run_lines and len(run_lines) >= 3:
            ...
            # Reference lines typically contain citation markers [N],
            # journal-style patterns (volume;issue:pages), or "et al."
            ref_line_count = sum(
...
- If the text does not mention survey weights, clustering adjustments, or sensitivity analyses for medication use, say "Verify whether survey weights / clustering / medication sensitivity analyses were applied" rather than "The study did not adjust for X".
- The absence of a description in the extracted material does not confirm the absence of the procedure in the study.
...
# Overall synopsis
# Major strengths
# Major considerations for closer scrutiny
# Statistical and methodological issues
# Points for closer inspection
# Issues to verify or clarify
...
- Prefer model-level evidence over a single paper-level label when the two conflict.
...
- Under "Major considerations for closer scrutiny", phrase points as substantial issues that may merit further checking, not as definitive flaws unless directly supported by the extracted evidence.
...
- Do not overclaim.
- In # Overall synopsis, do not compress multiple outcomes into a single directional sentence unless the supplied summaries show the same direction for all of them.
- If different outcomes point in different directions, summarise them separately in plain language, for example: "Outcome A was higher in X; Outcome B was higher in Y; Outcome C showed no clear difference."
...
- If the evidence manifest says a statistic type is present (e.g., "Confidence intervals reported: True"), do NOT make blanket claims that CIs are missing. You may note that CIs are not provided for every individual test, but frame this as "CIs are reported for some analyses but could not be identified for [specific test]", not "CIs are not reported".
- Apply the same principle to standard errors, p-values, effect sizes, and model fit statistics.
- For observational or survey data, distinguish between "verify whether X was handled" and "X was omitted". Say "Verify whether survey weights / clustering adjustments were applied" rather than "The study did not account for X" unless the text explicitly confirms the omission.
...
- Write as an internal critical appraisal memo to support human judgement, not as a final reviewer report to be pasted directly to authors or editors.
- Under "Points for closer inspection", phrase points as prompts for checking, verification, or closer reading.
- Under "Issues to verify or clarify", frame points as cautions or follow-up checks, not as directives to editors.
- Do not directly address authors unless the evidence is very clear; prefer formulations such as "Check whether...", "Verify whether...", or "Consider whether...".
...
- If the evidence manifest says a statistic type is present (e.g., "Confidence intervals reported: True"), actively soften any blanket claim that CIs are missing. Revise to note that CIs may not be provided for every individual test, but avoid claiming general absence.
- For observational or survey data, revise "The study did not adjust for X" to "Verify whether X was applied" unless the text explicitly confirms the omission.
...
_DEFAULT_CHAT_SYSTEM = """You are a helpful, knowledgeable assistant running locally on Apple Silicon.
Answer clearly and concisely.  When you are uncertain, say so."""
